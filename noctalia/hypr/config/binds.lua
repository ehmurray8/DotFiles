...
local function isTerminal(window)
    if not window or not window.class then
        return false
    end

    return terminalClasses[string.lower(window.class)] == true
end
...
hl.bind(mainMod .. " + C", function()
    ...
    if isTerminal(window) then
        sendShortcut(window, "CTRL + SHIFT", "C")
    else
        sendShortcut(window, "CTRL", "C")
    end
end)
...
hl.bind(mainMod .. " + V", function()
    ...
    if isTerminal(window) then
        sendShortcut(window, "CTRL + SHIFT", "V")
    else
        sendShortcut(window, "CTRL", "V")
    end
end)
...
hl.bind(
  ...
  hl.dsp.exec_cmd("~/.local/bin/audio-output-volume raise"),
  {
...
hl.bind(
  ...
  hl.dsp.exec_cmd("~/.local/bin/audio-output-volume lower"),
  {
...
hl.bind(
  "XF86AudioMute",
  hl.dsp.exec_cmd("~/.local/bin/audio-output-volume mute-toggle"),
  {
    description = "Mute",
    locked = true,
  }
...
hl.bind(
  ...
  hl.dsp.exec_cmd("~/.local/bin/audio-output-volume +1"),
  {
